...
<h1><img src="<?= base_url('assets/imgs/shop-cart-add-icon.png') ?>" class="header-img" style="margin-top:-3px;"> Thêm mới sản phẩm</h1>
<hr>
<?php
$timeNow = time();
...
<form method="POST" action="" enctype="multipart/form-data">
    ...
    foreach ($languages as $language) {
        ...
        <div class="locale-container locale-container-<?= $language->abbr ?>" <?= $language->abbr == MY_DEFAULT_LANGUAGE_ABBR ? 'style="display:block;"' : '' ?>>
            ...
            <div class="form-group">
                <a href="javascript:void(0);" class="btn btn-default showSliderDescrption" data-descr="<?= $i ?>">Flash sale? <span class="glyphicon glyphicon-circle-arrow-down"></span></a>
            </div>
            <div class="theSliderDescrption" id="theSliderDescrption-<?= $i ?>" <?= isset($_POST['in_slider']) && $_POST['in_slider'] == 1 ? 'style="display:block;"' : '' ?>>
                <div class="form-group">
                    <label for="basic_description<?= $i ?>">Mô tả ngắn cho sản phẩm flash sale </label>
            ...
            <div class="form-group">
                <label for="description<?= $i ?>">Mô tả sản phẩm </label>
            ...
            <div class="form-group for-shop">
                <label>Giá mới (*1000 VNĐ)</label>
                <input type="text" name="price[]" placeholder="Chỉ nhập số" value="<?= $trans_load != null && isset($trans_load[$language->abbr]['price']) ? $trans_load[$language->abbr]['price'] : '' ?>" class="form-control">
            </div>
            <div class="form-group for-shop">
                <label>Giá cũ (*1000 VNĐ)</label>
                <input type="text" name="old_price[]" placeholder="Chỉ nhập số" value="<?= $trans_load != null && isset($trans_load[$language->abbr]['old_price']) ? $trans_load[$language->abbr]['old_price'] : '' ?>" class="form-control">
            </div>
        </div>
        <?php
        $i++;
    ...
    <div class="form-group bordered-group">
        ...
        ?>
        <label for="userfile">Hình sản phẩm</label>
    ...
    <div class="form-group bordered-group">
        ...
        <a href="javascript:void(0);" data-toggle="modal" data-target="#modalMoreImages" class="btn btn-default">Thêm nhiều hình hơn</a>
    </div>
    <div class="form-group for-shop">
        <label>Số lượng</label>
        <input type="text" placeholder="số lượng" name="quantity" value="<?= isset($_POST['quantity']) ? htmlspecialchars($_POST['quantity']) : '' ?>" class="form-control" id="quantity">
    </div>
    ...
    <?php } ?>
    <div class="form-group for-shop">
        <label>Cho lên sale trên đầu trang?</label>
    ...
    <button type="submit" name="submit" class="btn btn-lg btn-default btn-publish" style="background-color: #00edcc;">Thêm</button>
